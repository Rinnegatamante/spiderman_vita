...
GLuint splash_tex[2];
int SplashRender(void *this) {
	static int inited = 0;
	if (!inited) {
		inited = 1;
		int w, h, n;
		void *buf = stbi_load("app0:splash.png", &w, &h, &n, 4);
		glGenTextures(2, splash_tex);
		glBindTexture(GL_TEXTURE_2D, splash_tex[0]);
		glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, w, h, 0, GL_RGBA, GL_UNSIGNED_BYTE, buf);
		free(buf);
		buf = stbi_load("app0:splash2.png", &w, &h, &n, 4);
		glBindTexture(GL_TEXTURE_2D, splash_tex[1]);
		glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, w, h, 0, GL_RGBA, GL_UNSIGNED_BYTE, buf);
		free(buf);
...
void patch_game(void) {
	...
	hook_addr(so_symbol(&spiderman_mod, "_ZN9GS_Splash6RenderEv"), (uintptr_t)&SplashRender);	
